...
//===----------------------------------------------------------------------===//
// Raising `arith.maxsi` / `arith.minsi` to `affine.max` / `affine.min`.
//===----------------------------------------------------------------------===//

// CHECK: #[[$MAP:.+]] = affine_map<()[s0, s1] -> (s0, s1)>
// CHECK-LABEL: @basic_max_symbols
// CHECK-SAME:  %[[A:.*]]: index, %[[B:.*]]: index
// CHECK:         %{{.*}} = affine.max #[[$MAP]]()[%[[A]], %[[B]]]
// CHECK-NOT:     arith.maxsi
func.func @basic_max_symbols(%a: index, %b: index) -> index {
  %r = arith.maxsi %a, %b : index
  return %r : index
}
...
// CHECK: #[[$MAP:.+]] = affine_map<()[s0, s1] -> (s0, s1)>
// CHECK-LABEL: @basic_min_symbols
// CHECK-SAME:  %[[A:.*]]: index, %[[B:.*]]: index
// CHECK:         %{{.*}} = affine.min #[[$MAP]]()[%[[A]], %[[B]]]
// CHECK-NOT:     arith.minsi
func.func @basic_min_symbols(%a: index, %b: index) -> index {
  %r = arith.minsi %a, %b : index
  return %r : index
}

// -----

// A mix of a dimension (the enclosing loop's induction variable) and a
// symbol (a top-level value) is categorized correctly in the resulting map.

// CHECK: #[[$MAP:.+]] = affine_map<(d0)[s0] -> (d0, s0)>
// CHECK-LABEL: @max_with_iv_dim_and_symbol
// CHECK-SAME:  %[[UB:.*]]: index, %[[K:.*]]: index
// CHECK:         affine.for %[[I:.*]] = 0 to %[[UB]] {
// CHECK:           %{{.*}} = affine.max #[[$MAP]](%[[I]])[%[[K]]]
func.func private @use(%x: index)

func.func @max_with_iv_dim_and_symbol(%ub: index, %k: index) {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  scf.for %i = %c0 to %ub step %c1 {
    %m = arith.maxsi %i, %k : index
    func.call @use(%m) : (index) -> ()
  }
  return
}
...
// Not raised: `%acc` is an `scf.for` iter_arg, neither a top-level value nor
// an affine induction variable, so it is not a valid affine dim/symbol.

// CHECK-LABEL: @max_not_raised_iter_arg
// CHECK:         arith.maxsi
// CHECK-NOT:     affine.max
func.func @max_not_raised_iter_arg(%init: index, %ub: index, %k: index) -> index {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  // TODO: is the for loop raised?
  %r = scf.for %i = %c0 to %ub step %c1 iter_args(%acc = %init) -> (index) {
    %m = arith.maxsi %acc, %k : index
    scf.yield %m : index
  }
  return %r : index
}
...
// `arith.maxui` / `arith.minui` are *not* raised: `affine.max` / `affine.min`
// evaluate their affine expressions as signed integers, so raising an
// unsigned comparison would change semantics for operands that are negative
// as signed values.
// TODO: Support raising unsigned.

// CHECK-LABEL: @maxui_not_raised
// CHECK:         arith.maxui
// CHECK-NOT:     affine.max
func.func @maxui_not_raised(%a: index, %b: index) -> index {
  %r = arith.maxui %a, %b : index
  return %r : index
}

// CHECK-LABEL: @minui_not_raised
// CHECK:         arith.minui
// CHECK-NOT:     affine.min
func.func @minui_not_raised(%a: index, %b: index) -> index {
  %r = arith.minui %a, %b : index
  return %r : index
}

// -----

// Integer (non-index) operands are cast to `index` first, mirroring the
// bound-casting behavior used for `scf.for` bounds: a cast per operand, the
// max/min computed in `index`, and the result cast back to the original
// type.

// CHECK: #[[$MAP:.+]] = affine_map<()[s0, s1] -> (s0, s1)>
// CHECK-LABEL: @max_index_cast
// CHECK-SAME:  %[[A:.*]]: i32, %[[B:.*]]: i32
// CHECK:         %[[A_IDX:.*]] = arith.index_cast %[[A]] : i32 to index
// CHECK:         %[[B_IDX:.*]] = arith.index_cast %[[B]] : i32 to index
// CHECK:         %[[R_IDX:.*]] = affine.max #[[$MAP]]()[%[[A_IDX]], %[[B_IDX]]]
// CHECK:         %[[R:.*]] = arith.index_cast %[[R_IDX]] : index to i32
// CHECK:         return %[[R]]
func.func @max_index_cast(%a: i32, %b: i32) -> i32 {
...
// Casts for the operands of a min/max that is not itself at the top level of
// the affine scope (here: inside a loop) are hoisted there, same as
// loop-bound casts.

// CHECK-LABEL:   func.func @min_index_cast_hoisted_out_of_loop(
// CHECK-SAME:      %[[A:.*]]: i32, %[[B:.*]]: i32, %[[UB:.*]]: index) {
// CHECK:           %[[A_IDX:.*]] = arith.index_cast %[[A]] : i32 to index
// CHECK:           %[[B_IDX:.*]] = arith.index_cast %[[B]] : i32 to index
// CHECK:           affine.for %{{.*}} = 0 to %[[UB]] {
// CHECK:             %{{.*}} = affine.min #{{.*}}()[%[[A_IDX]], %[[B_IDX]]]
func.func private @use_i32(%x: i32)

func.func @min_index_cast_hoisted_out_of_loop(%a: i32, %b: i32, %ub: index) {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  scf.for %i = %c0 to %ub step %c1 {
    %m = arith.minsi %a, %b : i32
    func.call @use_i32(%m) : (i32) -> ()
  }
  return
}
...
// Not raised: the index-cast would not be lossless (index pinned to 32 bits,
// operands are i64). Raised: same-width signed cast is lossless (index
// pinned to 32 bits, operands are i32).

// CHECK-LABEL: @maxsi_wider_than_index_not_raised
// CHECK:         arith.maxsi
// CHECK-NOT:     affine.max
// CHECK-LABEL: @maxsi_signed_same_width_raised
// CHECK:         affine.max
// CHECK-NOT:     arith.maxsi
module attributes {dlti.dl_spec = #dlti.dl_spec<#dlti.dl_entry<index, 32>>} {
  func.func @maxsi_wider_than_index_not_raised(%a: i64, %b: i64) -> i64 {
    %r = arith.maxsi %a, %b : i64
    return %r : i64
  }

  func.func @maxsi_signed_same_width_raised(%a: i32, %b: i32) -> i32 {
    %r = arith.maxsi %a, %b : i32
    return %r : i32
  }
}
...
func.func @for_lb_maxsi_index_cast_constant_step(%a: i32, %b: i32, %ub: i32) {
  %lb = arith.maxsi %a, %b : i32
  %step = arith.constant 1 : i32
  scf.for %i = %lb to %ub step %step : i32 {
    func.call @some_func(%i) : (i32) -> ()
  }
...
func.func @for_ub_minsi_constant_step(%k: index, %n: index) {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %ub = arith.minsi %k, %n : index
  scf.for %i = %c0 to %ub step %c1 {
    func.call @some_func(%i) : (index) -> ()
  }
  return
...
// CHECK: #[[$UB_MAP:.+]] = affine_map<()[s0, s1] -> ((s0 - s1 + 99) floordiv s0)>
// CHECK: #[[$IV_MAP:.+]] = affine_map<(d0, d1)[s0] -> (d0 + d1 * s0)>
// CHECK-LABEL:   func.func @max_lb_symbol_dynamic_step_from_arith(
// CHECK-SAME:      %[[STEP:.*]]: index, %[[K:.*]]: index, %[[N:.*]]: index) {
// CHECK:           %[[LB:.*]] = affine.max #[[$LB_MAP]]()[%[[K]], %[[N]]]
// CHECK:           affine.for %[[NEW_I:.*]] = 0 to #[[$UB_MAP]]()[%[[STEP]], %[[LB]]] {
// CHECK:             %[[OLD_I:.*]] = affine.apply #[[$IV_MAP]](%[[LB]], %[[NEW_I]])[%[[STEP]]]
// CHECK:             func.call @some_func(%[[OLD_I]]) : (index) -> ()
// CHECK-NOT:       scf.for
...
func.func @max_lb_symbol_dynamic_step_from_arith(%step: index, %k: index, %n: index) {
  %ub = arith.constant 100 : index
...
// The `arith.maxsi` itself is still raised to `affine.max` even when the
// surrounding (dynamic-step) `scf.for` cannot be raised because its lower
// bound depends on a non-top-level value (the outer loop's induction
// variable) -- mirroring `max_lb_iv_dynamic_step_not_raised` above, but
// starting from `arith.maxsi` instead of a hand-written `affine.max`.

// CHECK-LABEL: @max_lb_iv_dynamic_step_not_raised_from_arith
...
func.func @max_lb_iv_dynamic_step_not_raised_from_arith(%n: index, %ub: index, %k: index,
                                                          %step: index) {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  scf.for %i = %c0 to %n step %c1 {
...
// An operand defined *above* the affine scope (here: a block argument of the
// function enclosing the `test.affine_scope`) is a legal affine symbol inside
// the scope, so the min/max is raised. The cast is placed next to the
// definition of the operand -- hoisting it to the top of the affine scope
// would not make it any more legal than leaving it at the use.

// CHECK-LABEL:   func.func @max_operand_defined_above_affine_scope(
// CHECK-SAME:      %[[A:.*]]: i32, %[[B:.*]]: i32
// CHECK:           %[[A_IDX:.*]] = arith.index_cast %[[A]] : i32 to index
// CHECK:           %[[B_IDX:.*]] = arith.index_cast %[[B]] : i32 to index
// CHECK:           test.affine_scope {
// CHECK:             %{{.*}} = affine.max #{{.*}}()[%[[A_IDX]], %[[B_IDX]]]
// CHECK-NOT:     arith.maxsi
func.func @max_operand_defined_above_affine_scope(%a: i32, %b: i32) {
  test.affine_scope {
...
// Not raised: `%acc` is an `scf.for` iter_arg, so it is not a valid affine
// symbol and no cast for it can be made legal. The raisable operand `%a` must
// not have a cast materialized for it either: a pattern that fails to match
// must leave the IR untouched.

// CHECK-LABEL: @max_int_partially_raisable_not_rewritten
// CHECK-NOT:     arith.index_cast
// CHECK:         arith.maxsi
// CHECK-NOT:     affine.max
// CHECK-NOT:     arith.index_cast
func.func @max_int_partially_raisable_not_rewritten(%a: i32, %init: i32, %ub: index) -> i32 {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %r = scf.for %i = %c0 to %ub step %c1 iter_args(%acc = %init) -> (i32) {
    %m = arith.maxsi %a, %acc : i32
    scf.yield %m : i32
  }
  return %r : i32
}
